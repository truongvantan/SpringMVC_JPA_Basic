...
<body>
	...
	<c:set var="idCategory"
		value='${not empty param.idCategory ? param.idCategory : "" }' />
	...
	int stt = (currentPageNumer - 1) * 10 + 1;
			...
			<div class="panel-heading">
				<form action="${contextPath}/product/search" method="POST"
					...
					<div class="col col-xs-4">
						<select class="form-control w-75 d-inline-block" name="category"
							id="listCategories">
						...
						<c:forEach var="product" items="${listProducts}">
							<tr class="d-flex">
								<td
									...
									<div>
										<a href="${contextPath}/product/showEdit?idProduct=${product.id}"
											class="btn btn-success mr-1" data-toggle="tooltip"
											title="Chỉnh sửa"> <i class="fa-solid fa-pencil fa-xs"></i>
										</a> <a onclick="return confirmDeleteProduct('${product.id}');"
											href="${contextPath}/product/delete?idProduct=${product.id}"
								...
								<td class="col d-flex align-items-center"><fmt:formatNumber
										type="number" pattern="###,###.00" value="${product.priceIn}" /></td>
								<td class="col d-flex align-items-center"><fmt:formatNumber
										type="number" pattern="###,###.00" value="${product.priceSell}" /></td>
								<td class="col d-flex justify-content-center align-items-center">
									<img src="${product.linkImage}" height="80" alt="product image">
								</td>
						...
						<nav aria-label="Page navigation example"
							...
							<ul class="pagination">
								...
								<li class="page-item"><a class="page-link"
									href='${functionPrefix}?idCategory=${idCategory}&page=1'>Đầu</a></li>
								<li class="page-item"><a class="page-link"
									href='${functionPrefix}?idCategory=${idCategory}&page=<%=currentPageNumer - 1%>'>Trước</a></li>
								<%
								}
								%>
								...
								%>
								<li class="page-item"><a class="page-link"
									href='${functionPrefix}?idCategory=${idCategory}&page=<%=pageNumberList[k]%>'><%=pageNumberList[k]%></a></li>
								<%
								}
								%>
								...
								%>
								<li class="page-item"><a class="page-link"
									href='${functionPrefix}?idCategory=${idCategory}&page=<%=currentPageNumer + 1%>'>Tiếp</a></li>
								<li class="page-item"><a class="page-link"
									href='${functionPrefix}?idCategory=${idCategory}&page=<%=totalPageNumber%>'>Cuối</a></li>
								<%
								}
								%>
							</ul>
						...
						function() {
							$('#listCategories')
									.on(
											'change',
											function() {
												location.href = '${contextPath}/product/show?idCategory='
														+ $(this).val();
											});

							$('#listCategories').val('${param.idCategory}');
